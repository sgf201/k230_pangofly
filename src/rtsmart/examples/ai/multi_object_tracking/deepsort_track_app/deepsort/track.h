...
#include <chrono>   // steady_clock、duration 等
#include <algorithm> 
#include "dataType.h"
...
public:
    Track(KAL_MEAN& mean, KAL_COVA& covariance, int track_id,
          int n_init, int max_age, const FEATURE& feature,float score);
    ...
    double start_time = 0.0;   // 该目标第一次出现的时间戳
    double stay_time = 0.0;    // 当前累计驻留时间
