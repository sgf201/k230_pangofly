...
Track::Track(KAL_MEAN &mean, KAL_COVA &covariance, int track_id, int n_init, int max_age, const FEATURE &feature,float score)
{
    this->mean = mean;
    ...
    this->start_time = duration_cast<duration<double>>(steady_clock::now().time_since_epoch()).count();
    this->stay_time = 0.0f;
}
