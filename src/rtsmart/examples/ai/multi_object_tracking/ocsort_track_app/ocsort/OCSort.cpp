...
namespace ocsort {
    ...
    std::vector<Eigen::RowVectorXf> OCSort::update(Eigen::MatrixXf dets) {
        ...
        for (int i : unmatched_dets) {
            ...
            trackers.push_back(std::make_unique<KalmanBoxTracker>(tmp_bbox, cls_, delta_t));
        ...
        for (int i = trackers.size() - 1; i >= 0; i--) {
            ...
            if (trackers.at(i)->time_since_update < 1 && ((trackers.at(i)->hit_streak >= min_hits) | (frame_count <= min_hits))) {
                trackers.at(i)->stay_time = duration_cast<duration<double>>(steady_clock::now().time_since_epoch()).count() - trackers.at(i)->start_time;
                Eigen::RowVectorXf tracking_res(8);
                tracking_res << d(0), d(1), d(2), d(3), trackers.at(i)->id + 1, trackers.at(i)->cls, trackers.at(i)->conf, trackers.at(i)->stay_time;
