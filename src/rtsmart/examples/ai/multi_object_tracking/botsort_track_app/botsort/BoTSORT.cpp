...
BoTSORT::BoTSORT(bool reid_enabled,float track_high_thresh,float track_low_thresh,float new_track_thresh,long track_buffer,float match_thresh,float proximity_thresh,float appearance_thresh,long frame_rate,float lambda)
{
    TrackerParams tracker_params;
    tracker_params.reid_enabled = reid_enabled;
    tracker_params.track_high_thresh = track_high_thresh;
    tracker_params.track_low_thresh = track_low_thresh;
    tracker_params.new_track_thresh = new_track_thresh;
    tracker_params.track_buffer = track_buffer;
    tracker_params.match_thresh = match_thresh;
    tracker_params.proximity_thresh = proximity_thresh;
    tracker_params.appearance_thresh = appearance_thresh;
    tracker_params.frame_rate = frame_rate;
    tracker_params.lambda = lambda;
    _load_params_from_config(tracker_params);

    // Tracker module
    _frame_id = 0;
    _buffer_size = static_cast<long>(_frame_rate / 30.0 * _track_buffer);
    _max_time_lost = _buffer_size;
...
BoTSORT::track(const std::vector<Detection> &detections)
{
    ...
    if (!detections.empty())
    {
        for (Detection &detection: const_cast<std::vector<Detection> &>(detections))
        {
            ...
            detection.bbox_tlwh.width =detection.bbox_tlwh.width;
            detection.bbox_tlwh.height =detection.bbox_tlwh.height;
            ...
            if (detection.confidence > _track_low_thresh)
            {
                if (_reid_enabled)
                {
                    FeatureVector embedding = Eigen::Map<FeatureVector>(detection.feature.data());
                    tracklet = std::make_shared<Track>(tlwh, detection.confidence, detection.class_id,embedding,200);
                }
                else
