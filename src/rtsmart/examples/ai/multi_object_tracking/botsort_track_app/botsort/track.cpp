      ...
      tracklet_len(0), is_activated(false), state(TrackState::New)
{
    ...
    start_time = duration_cast<duration<double>>(steady_clock::now().time_since_epoch()).count();
    stay_time = 0.0f;
                   ...
                   uint32_t frame_id)
{
    ...
    stay_time = duration_cast<duration<double>>(steady_clock::now().time_since_epoch()).count() - start_time;
}
