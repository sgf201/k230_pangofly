#!/bin/bash
# Build and package  K230 opencv programs (incremental build)
# Function: Use CMake and cross-compile toolchain to build all opencv examples in build/bin and collect utility files
...
collect_outputs() {
    if [ -d "${BUILD_DIR}/bin" ]; then
        ...
        echo "[WARN] Build output directory not found: ${BUILD_DIR}/bin"
    fi
}
