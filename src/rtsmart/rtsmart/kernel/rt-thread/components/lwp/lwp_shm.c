...
/* Check if the virtual address is already mapped to the given physical address */
static void *_lwp_find_existing_mapping(struct rt_lwp *lwp, void *shm_vaddr, size_t shm_pa, size_t shm_size)
{
    void *va = RT_NULL;
    void *mapped_pa = RT_NULL;
    size_t page_size;
    ...
    for (size_t offset = 0; offset < shm_size; offset += page_size)
    {
        mapped_pa = rt_hw_mmu_v2p(&lwp->mmu_info, (void *)((size_t)shm_vaddr + offset));
        if (!mapped_pa)
        {
            return RT_NULL;
        }
        if ((size_t)mapped_pa != shm_pa + offset)
        {
            return RT_NULL;
        }
    }
...
static void *_lwp_shmat(int id, void *shm_vaddr)
{
    ...
    if (shm_vaddr)
    {
        va = _lwp_find_existing_mapping(lwp, shm_vaddr, p->addr, p->size);
        if (va)
        {
            p->ref++;
            return va;
...
int _lwp_shmdt(void *shm_vaddr)
{
    ...
    p = _lwp_shm_struct_get(lwp, shm_vaddr);
    if (!p)
    {
        return -1;
    }
