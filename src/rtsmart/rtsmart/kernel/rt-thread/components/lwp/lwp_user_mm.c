        ...
        int flags, int fd, off_t pgoffset)
{
    ...
    rt_kprintf("[lwp_mmap2] START: addr=%p, length=%u, prot=%d, flags=%x, fd=%d, offset=%llu\n",
                addr, (unsigned int)length, prot, flags, fd, (unsigned long long)pgoffset);

    if (fd == -1)
    {
        rt_kprintf("[lwp_mmap2] fd == -1, anonymous mapping\n");
        rt_mm_lock();
        ...
        if (ret)
        {
            ...
            rt_kprintf("[lwp_mmap2] anonymous map success: %p\n", ret);
        }
        else
        {
            ret = (void *)-1;
            rt_kprintf("[lwp_mmap2] anonymous map failed\n");
        }
    }
    else
    {
        ...
        rt_kprintf("[lwp_mmap2] fd=%d, calling fd_get...\n", fd);
        d = fd_get(fd);
        
        if (d)
        {
            rt_kprintf("[lwp_mmap2] fd_get succeeded, d=%p\n", d);
            rt_kprintf("[lwp_mmap2] fnode=%p\n", d->fnode);
            
            if (d->fnode)
            {
                rt_kprintf("[lwp_mmap2] file type: %d\n", d->fnode->type);
                
                if (d->fnode->type == 2)  /* FT_DEVICE is typically 2 */
                {
                    rt_kprintf("[lwp_mmap2] File is device type\n");
                    ...
                    {
                        ret = mmap2.ret;
                        rt_kprintf("[lwp_mmap2] device mmap success: %p\n", ret);
                    }
                    else
                    {
                        rt_kprintf("[lwp_mmap2] device mmap failed\n");
                        ret = (void *)-1;
                    }
                }
                else
                {
                    rt_kprintf("[lwp_mmap2] File is regular file type (not device)\n");
                    void *map_result;
                    
                    rt_kprintf("[lwp_mmap2] Calling lwp_map_user with addr=%p, length=%u\n", addr, (unsigned int)length);
                    
                    rt_mm_lock();
                    map_result = lwp_map_user(lwp_self(), addr, length, 0);
                    rt_mm_unlock();
                    
                    rt_kprintf("[lwp_mmap2] lwp_map_user returned: %p\n", map_result);
                    
                    if (map_result)
                    {
                        rt_kprintf("[lwp_mmap2] map_result is not NULL\n");
                        
                        if (addr != NULL && (flags & 0x10) != 0)
                        {
                            rt_kprintf("[lwp_mmap2] MAP_FIXED flag set, checking address match\n");
                            if (map_result != addr)
                            {
                                rt_kprintf("[lwp_mmap2] fixed addr mismatch: req=%p, got=%p\n", addr, map_result);
                                lwp_unmap_user(lwp_self(), map_result);
                                ret = (void *)-1;
                            }
                            else
                            {
                                ret = map_result;
                                rt_kprintf("[lwp_mmap2] fixed map success: %p\n", ret);
                            }
                        }
                        else
                        {
                            ret = map_result;
                            rt_kprintf("[lwp_mmap2] non-fixed map success: %p\n", ret);
                        }
                    }
                    else
                    {
                        ret = (void *)-1;
                        rt_kprintf("[lwp_mmap2] file map failed - lwp_map_user returned NULL\n");
                    }
                }
            }
            else
            {
                rt_kprintf("[lwp_mmap2] d->fnode is NULL\n");
                ret = (void *)-1;
            }
        }
        else
        {
            rt_kprintf("[lwp_mmap2] fd_get FAILED for fd=%d\n", fd);
            ret = (void *)-1;
        }
    }

    rt_kprintf("[lwp_mmap2] END: return=%p\n", ret);
    return ret;
}
